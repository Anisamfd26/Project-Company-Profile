          ...
          <Tabs defaultValue="all" className="w-full" onValueChange={handleTabChange}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                {filteredNews.map((item) => (
                  <div key={item.id} className="relative group rounded-xl overflow-hidden shadow-lg">
                    <img src={item.imagePath || 'https://images.unsplash.com/photo-1505373877841-8d25f7d46678?w=800'} alt={item.title} className="w-full h-full object-cover transition-transform duration-500 group-hover:scale-110" />
            ...
            {filteredNews.length === 0 && (
              <p className="text-center text-gray-500 mt-8">Tidak ada berita atau acara dalam kategori ini.</p>
            )}
          </Tabs>
        </div>
      </section>
